...
export async function pullSecrets(
  ...
  if (!vaultNameOrId) {
    error(
      "No vault specified and no project configuration found. Run `vaultix init` or specify a vault.",
    );
    return [];
  }

  // 1. Get the vault member record via API
  const { data: memberData, error: memberError } = await callCliApi(
    "get-vault-access",
    { vaultNameOrId },
  );
  ...
  if (memberError || !memberData) {
    error(
      `Failed to fetch vault access: ${memberError || "No access to this vault"}`,
    );
    return [];
  }
  ...
  const { data: envData, error: envError } = await callCliApi(
    "get-environment",
    {
  ...
  try {
    ...
    error(
      "Failed to decrypt vault key. Your local private key might not match.",
    );
    return [];
  }

  // 4. Fetch the secrets via API
  const { data: secretsData, error: secretsError } = await callCliApi(
    "get-secrets",
    {
      vaultId: member.vault_id,
  ...
  if (!secretsData || secretsData.length === 0) {
    warn(
      `No secrets found for vault "${vaultNameOrId}" and environment "${env}"`,
    );
    return [];
  }
  ...
  for (const s of secrets) {
    try {
      const value = await decryptSecretValue(
        s.encrypted_payload,
        s.nonce,
        vaultKey,
      );
