...
export async function init(): Promise<void> {
  ...
  if (!config.token) {
    error(
      "You must be logged in to initialize a project. Run `vaultix login` first.",
    );
    return;
  }

  info("Fetching your vaults...");
  const { data: vaultsData, error: apiError } =
    await callCliApi("get-user-vaults");

  if (apiError) {
    error(`Failed to fetch vaults: ${JSON.stringify(apiError)}`);
    return;
  }
  ...
  vaults.forEach((v, i) => {
    console.log(
      `  ${colors.fg.cyan}${i + 1}.${colors.reset} ${v.vaults.name} ${colors.dim}(${v.vault_id})${colors.reset}`,
    );
  });
  ...
  console.log("");
  success(
    `Project successfully attached to vault: ${bold(selected.vaults.name)}`,
  );
